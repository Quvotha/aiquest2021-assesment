...
class ZipCode(object):
    ...
    def normalize(cls, zipcode: Any) -> str:
        ...
        try:
            ...
            if decimal_part != 0:
                return cls.NOT_ZIP_CODE
            zipcode = zipcode if integer_part < 10000 else str(int(integer_part))
        except ValueError:
    ...
    @classmethod
    def first_digit_of(cls, zipcode: str, city: str) -> str:
        """Get or impute first digit of `zipcode`

        Parameters
        ----------
        zipcode : str
            Zipcode, expected to be normalized by `Zipcode.normalize`.
        city : str
            City name, used for imputation.

        Returns
        -------
        1st digit: str
            Extracted or imputed 1st digit of zipcode.
        """
        if not isinstance(zipcode, str):
            return cls.NOT_ZIP_CODE
        else:
...
class ZipCodePreprocessor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        ...
        ----------
        X : pd.DataFrame
            Must have 2 columns: `city` and `zipcode`.
        y : [type], optional
        ...
        X = X[['city', 'zipcode']].dropna()
        X['zipcode_normalized'] = np.vectorize(ZipCode.normalize(X['zipcode']))
        X['zipcode_1st_digit'] = np.vectorize(ZipCode.first_digit_of)(X['zipcode'], X['city'])
        self.mode_ = {first_digit: df['zipcode'].mode().min()
                      for first_digit, df in X.groupby('zipcode_1st_digit')}
        return self

    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        out_df = pd.DataFrame()
        zipcode_normalized = np.vectorize(ZipCode.normalize)(X['zipcode'])
        out_df['zipcode_1st_digit'] = np.vectorize(ZipCode.first_digit_of)(
            zipcode_normalized,
            X['city']
        ).astype('object')
        out_df['zipcode5'] = np.vectorize(self._get_five_digit_code)(
            out_df['zipcode_1st_digit'],
            zipcode_normalized
        ).astype('object')
        out_df['zipcode_imputed'] = (
            (zipcode_normalized == ZipCode.NOT_ZIP_CODE) * 1).astype('uint8')
        return out_df

    def _get_five_digit_code(self, first_digit: str, zipcode: str) -> str:
        return self.mode_[str(first_digit)][:5] if zipcode == ZipCode.NOT_ZIP_CODE else zipcode[:5]
